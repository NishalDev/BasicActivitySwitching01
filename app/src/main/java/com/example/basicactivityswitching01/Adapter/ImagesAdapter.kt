...
class ImagesAdapter(val context: Context, val images: List<Images>, val itemClick : (Images) -> Unit): RecyclerView.Adapter<ImagesAdapter.Holder>() {
    inner class Holder(itemView: View,val itemClick: (Images)-> Unit): RecyclerView.ViewHolder(itemView) {
